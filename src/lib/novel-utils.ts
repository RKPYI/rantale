...
export function simplifyPagination<T>(paginatedResponse: PaginatedResponse<T>): SimplePagination {
  return {
...
export function filterNovelsByGenre(novels: Novel[], genreSlug: string): Novel[] {
...
export function sortNovels(novels: Novel[], sortBy: 'rating' | 'views' | 'chapters' | 'updated' | 'created', order: 'asc' | 'desc' = 'desc'): Novel[] {
...
export function calculateAverageRating(novels: Novel[]): number {
  ...
  const total = novels.reduce((sum, novel) => sum + parseFloat(novel.rating || '0'), 0);
  return total / novels.length;
}

// Get novels by status
export function getNovelsByStatus(novels: Novel[], status: 'ongoing' | 'completed' | 'hiatus'): Novel[] {
