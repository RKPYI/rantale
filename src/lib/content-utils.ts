...
// Format edit time for display
export function formatEditTime(editedAt: string): string {
  ...
  if (diffInSeconds < 3600)
    return `edited ${Math.floor(diffInSeconds / 60)}m ago`;
  if (diffInSeconds < 86400)
    return `edited ${Math.floor(diffInSeconds / 3600)}h ago`;
  if (diffInSeconds < 2592000)
    return `edited ${Math.floor(diffInSeconds / 86400)}d ago`;
  return `edited ${date.toLocaleDateString()}`;
}

// Get comment last modified display text
export function getCommentModifiedText(comment: Comment): string {
  if (isCommentEdited(comment) && comment.edited_at) {
    return formatEditTime(comment.edited_at);
  }
  return "";
}

// Get full comment timestamp info for display
export function getCommentTimestampInfo(comment: Comment): {
  created: string;
  createdRaw: string;
  edited?: string;
  editedRaw?: string;
  ...
  const info = {
    created: formatCommentTime(comment.created_at),
    createdRaw: comment.created_at,
    isEdited: isCommentEdited(comment),
  };

  if (info.isEdited && comment.edited_at) {
    return {
      ...info,
      edited: formatEditTime(comment.edited_at),
      editedRaw: comment.edited_at,
    };
  }
...
export function sortComments(
  ...
  return [...comments].sort((a, b) => {
    switch (sortBy) {
      ...
      case "controversial":
        const aRatio =
          Math.min(a.likes, a.dislikes) / Math.max(a.likes, a.dislikes, 1);
        const bRatio =
...
export function getProgressStatusMessage(
