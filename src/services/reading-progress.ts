...
export const readingProgressService = {
  ...
  async getNovelProgress(novelSlug: string): Promise<ReadingProgressResponse> {
    const response = await apiClient.get<ReadingProgressResponse>(`/reading-progress/${novelSlug}`);
    return response.data;
  },

  // Update reading progress (requires authentication)
  async updateProgress(data: UpdateReadingProgressRequest): Promise<ReadingProgressResponse> {
    const response = await apiClient.put<ReadingProgressUpdateResponse>('/reading-progress', data);
    return response.data.progress;
  },

  // Create initial reading progress (requires authentication)
  async createInitialProgress(data: CreateReadingProgressRequest): Promise<ReadingProgressResponse> {
    const response = await apiClient.post<ReadingProgressCreateResponse>('/reading-progress', data);
    return response.data.progress;
  },
  ...
  async getUserReadingProgress(): Promise<UserReadingProgressResponse> {
    const response = await apiClient.get<UserReadingProgressResponse>('/reading-progress/user');
    return response.data;
  },

  // Delete reading progress for a novel (requires authentication)
  async deleteProgress(novelSlug: string): Promise<string> {
    const response = await apiClient.delete<MessageResponse>(`/reading-progress/${novelSlug}`);
  ...
  async continueReading(novelSlug: string, chapterNumber: number): Promise<ReadingProgressResponse> {
    return this.updateProgress({ novel_slug: novelSlug, chapter_number: chapterNumber });
  },
};