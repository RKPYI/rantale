...
export const authService = {
  ...
  async resendEmailVerification(): Promise<{ message: string }> {
    const response = await apiClient.post<{ message: string }>('/auth/email/resend-verification');
    return response.data;
  },

  async verifyEmail(id: string, hash: string): Promise<{ message: string }> {
    const response = await apiClient.post<{ message: string }>(`/auth/email/verify/${id}/${hash}`);
    return response.data;
  },
